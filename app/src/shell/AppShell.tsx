...
export function AppShell({
  ...
  const footerInitial =
    account?.avatarInitials?.slice(0, 1) ?? account?.name.slice(0, 1) ?? "A";
  const footerName = account?.name ?? "Account";
  const footerMeta = account
    ? `${account.plan} · ${cronlets?.length ?? 0} cronlets`
    : tc("shell.providerMissing");
      ...
      <LanguageContext.Provider value={{ language, setLanguage, tc }}>
        <div className="app">
          <aside className="sidebar">
            <Link to={`/${demoSearch}`} className="brand">
              <span className="brand-mark">
                <Check size={15} />
              </span>
              <span>
                My<span className="brand-cron">Cron</span>
              </span>
            </Link>
            ...
            <div className="nav-link">
              <Icon name="cube" /> {tc("nav.agents")}
            </div>
            <div className="nav-link">
              <Icon name="layers" /> {tc("nav.runtimes")}
            </div>
            <Link to={`/account${demoSearch}`} className="sidebar-user">
              <div className="avatar">{footerInitial}</div>
              <div>
                <b style={{ color: "var(--ink)" }}>{footerName}</b>
                <br />
                <span className="mono">{footerMeta}</span>
              </div>
            </Link>
          </aside>
          <main className="main">
            <div className="topbar">
              <button
                ...
                onClick={() =>
                  showNotice(
                    tc("notice.searchTitle"),
                    tc("notice.searchDetail"),
                  )
                }
                aria-label={tc("shell.search")}
              >
                <Search size={16} /> {tc("shell.search")}{" "}
              ...
              <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <div className="live-pill">
                  <span className="dot" />{" "}
                  {demoMode ? tc("shell.runningNow") : tc("shell.previewMode")}
                </div>
                <button
                  className="ghost-btn"
          ...
          <nav className="phone-nav">
            ...
            />
            <MobileNav
              to={`/account${demoSearch}`}
              icon="user"
              label={tc("nav.account")}
            />
          </nav>
        </div>
