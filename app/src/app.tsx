    ...
    <div className="app">
      ...
      <main className="main">
        <div className="topbar">
          <div className="search"><Search size={16}/> Search cronlets, runs, evidence... <span style={{ marginLeft: "auto" }} className="mono">⌘K</span></div>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            ...
            <button className="ghost-btn" aria-label="Notifications"><Bell size={16}/></button>
          </div>
        </div>
...
function BuilderSurface() {
  ...
  async function submit() { setSaving(true); setError(null); try { const c = await actions.create({ ...draft, requiredEvidence: evidence }); navigate(`/cronlets/${c.id}${demoSearch()}`); } catch (e) { setError((e as Error).message); } finally { setSaving(false); } }
  return <>
...
function AccountSurface() {
  ...
  return <><PageHead eyebrow="Workspace" title="Account" sub="Manage the workspace boundary for scheduled agent work." action={<button className="ghost-btn"><Bell size={16}/></button>} /><div className="list" style={{ maxWidth: 720 }}><div className="card account-card"><div className="avatar" style={{ width: 60, height: 60, borderRadius: 14 }}>{account.avatarInitials ?? account.name.slice(0,1)}</div><div><h2 style={{ margin: 0 }}>{account.name}</h2><div className="muted">{account.email}</div></div><span className="status verified">{account.plan}</span><button className="ghost-btn">Profile & identity → Edit</button></div>{budget && <div className="card card-pad"><div style={{ display: "flex", justifyContent: "space-between" }}><h3>Compute budget</h3><span className="mono">renews {budget.renewsLabel}</span></div><div className="budget-line"><strong>{budget.usedLabel}</strong><span className="muted">of {budget.limitLabel} this cycle</span></div><div className="progress"><span style={{ width: `${budget.usedFraction*100}%` }}/></div><p className="mono">{budget.routines} routines · {budget.runsPerWeek} runs / wk <span style={{ float: "right", color: "var(--green)" }}>{Math.round(budget.usedFraction*100)}% used</span></p><button className="ghost-btn">Manage subscription & invoices →</button></div>}<div className="card"><div className="card-pad"><h3>Alerts</h3></div>{alerts?.map((alert) => <AlertRow key={alert.key} alert={alert} toggle={async () => { await actions.setAlertPreference(alert.key, !alert.enabled); reload(); }} />)}</div></div></>;
}
function AlertRow({ alert, toggle }: { alert: AlertPreference; toggle: () => void }) { return <div className="alert-row"><span className="iconbox" style={{ width: 34, height: 34 }}><Bell size={16}/></span><div className="grow"><b>{alert.label}</b><br/><span className="muted">{alert.detail}</span></div><button className={`switch ${alert.enabled ? "on" : ""}`} onClick={toggle} aria-label={`Toggle ${alert.label}`}><span/></button></div>; }
