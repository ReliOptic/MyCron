...
export type StoreData = { cronlets: CronletRecord[]; approvals: ApprovalRecord[]; runs: RunRecord[]; evidence: EvidenceRecord[]; audit: AuditEvent[] };

export type Store = {
  data: StoreData;
  save(): void;
  nextId(prefix: "crn" | "apr" | "run" | "ev" | "aud"): string;
};
...
function readData(path: string): StoreData {
  if (!existsSync(path)) {
    return { cronlets: [], approvals: [], runs: [], evidence: [], audit: [] };
  }
...
function normalizeData(data: Partial<StoreData>): StoreData {
  return { cronlets: data.cronlets ?? [], approvals: data.approvals ?? [], runs: data.runs ?? [], evidence: data.evidence ?? [], audit: data.audit ?? [] };
}
...
function nextNumber(data: StoreData, prefix: string): number {
  const ids = [...data.cronlets, ...data.approvals, ...data.runs, ...data.evidence, ...data.audit].map(item => item.id);
